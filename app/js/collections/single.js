  ...
  calculate: function() {
    ...
    Cals = [
      ...
      function(plates, i, j) {
        ...
        //将其余共有单铰加入新刚片的多余约束中
        var djcontrs = _.flatten(_.pluck(_.where(newplate.innercontrs, {
          type: "dj"
        }), "order"));
      ...
      function(plates, i, j) {
        ...
        _.each(newplatedjs, function(newplatedj) {
          _.each(nextplatedjs, function(nextplatedj) {
            _.each(plates, function(plate, index) {
              if (!_.isEqual(plate.components, _.union(plate.components, newplate.components)) && !_.isEqual(plate.components, _.union(plate.components, nextplate.components))) {
                if (_.contains(plate.connects, newplatedj) && _.contains(plate.connects, nextplatedj)) {
                  threes.push({
